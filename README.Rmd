...
gdfWarpf<- pullNHLAPI("Team",aName = "Devils")
gdfWarpg<- pullNHLAPI("Team",aTeamID = 2)
gdfWarph<- pullNHLAPI("Team",aFranchiseID = "2")
gdfWarpi<- pullNHLAPI("Team")
gdfWarpf<- pullNHLAPI("Team")


```

##Example Analysis  
Haven't followed Hockey too much, in my life, but Wayne Gretzky was a household name growing up.  Let's do some exploration to get some context for how good of a player he was.

Do a join -- will join gretzky records with franchise records
create two new variables -- a) skater gretzky y/n b) some derived stats / games played metric?
some contingency tables 
create numerical summaries on a categorical variable



##Downselect to Gretzky's franchises & plot where he is relative to other skaters.
##Pull skater data.  Create a y/n for being wayne gretzky 

```{r Analysis}
gTibGretz<- as_tibble(pullNHLAPI("FranchiseSkaterRecords")) %>% 
  mutate(ifelse(data.lastName=="Gretzky"))

gGretzRecords<- as_tibble(gdfFranSkat)%>% filter (data.lastName == "Gretzky" & data.firstName == "Wayne")
gGretzFactor<- as_tibble(gdfFranSkat)%>% filter (data.lastName == "Gretzky" & data.firstName == "Wayne")
```

...
gTibPositionSummary2<- gTibGretz %>% group_by(data.positionCode) %>% 
  summarise(nPlayerFranchiseObs = n(), avgGoalsPerGame=mean(goalsPerGame),
            stdDevGoals = sd(goalsPerGame), avgPenaltyBoxPerGame = mean(penaltyMinPerGame),
            stdDevPenaltyBoxPerGame = sd(penaltyMinPerGame))
...
```
Clearly an exemplary player with numbers that are matched by few across his tenures at several franchises.  Let's see how his records compare to his franchises

```{r Analysis_2}
#Figure out which teams Gretzky Played on, and filter data table down to those franchises
gGretzFranchList<- gTibGretz %>% group_by(Gretzky, data.franchiseId, data.franchiseName) %>%
  summarise(count = n())%>%filter(Gretzky =="Wayne Gretzky")

#Print the teams
kable(gGretzFranchList%>%select(-count))

#Filter skater list down to those franchises
gdfPlayersOnGretzFranchises<-gTibGretz%>% 
  filter(data.franchiseId %in% gGretzFranchList$data.franchiseId)

#Join to Franchise Totals
gdfGretzkyFranch<-gdfPlayersOnGretzFranchises%>%
  inner_join(pullNHLAPI("FranchiseTotals"), by = "data.franchiseId")
...
Do a join -- will join gretzky records with franchise records
some contingency tables -- position vs franchise
create numerical summaries on a categorical variable -- average goals and penalty time vs franchise




##Join to overall franchise records 
